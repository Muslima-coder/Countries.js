...
function handleMoreBtnClick (id){
  ...
  <div class="w-[40%] text-end">
      <strong class="font-extrabold inline-block text-[10px] sm:text-[16px] text-[var(--color-text)] mb-[16px]">${findObj.name}</strong>
       <p class="text-[var(--color-text)]"> <span class="font-semibold text-[10px]  sm:text-[14px] leading-[16px] text-[var(--color-text)]">Population: </span> ${findObj.population.toLocaleString("ru-RU")}</p>
      <p class="text-[var(--color-text)]"> <span class="font-semibold text-[10px] sm:text-[14px] leading-[16px] text-[var(--color-text)]">Region: </span> ${findObj.name}</p>
      <p class="text-[var(--color-text)]"> <span class="font-semibold text-[10px] sm:text-[14px] leading-[16px] text-[var(--color-text)]">Capital: </span> ${findObj.capital}</p>
  </div>
  </div>
  `
}
